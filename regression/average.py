...
class AverageTripLearner(object):
    ...
    def __call__(self, data):
        #TUKAJ POLIČEM LINEARNO REGRESIJO???
...
if __name__ == "__main__":
    ...
    f = gzip.open("train.txt.csv.gz", "rt") #ucenje
    reader = csv.reader(f, delimiter="\t")
    next(reader)
    data = [ d for d in reader ]

    #model each line separately with AverageTripLearner
    l = SeparateBySetLearner(AverageTripLearner()) #za vsako ID izracuna povp cas voznje
    c = l(data)
    #print(c)

    for d in data:
        pass
        #print(linekey(d), c(d)) #linekey(d)==linija, smer, zadnja postaja, c(d)==povprečna pot

    f = gzip.open("test.txt.csv.gz", "rt") #odprem file, kjer prihod ni znan iz naucenega izracunam prhod, ki je zapisan v average2.txt
    reader = csv.reader(f, delimiter="\t")
    next(reader) #skip legend

    fo = open("average.txt", "wt")
    for l in reader:
        fo.write(lpputils.tsadd(l[-3], c(l)) + "\n") #na podlagi odhoda l[-3] doda izracunane sekunde iz modela (dolzina poti)
    fo.close()